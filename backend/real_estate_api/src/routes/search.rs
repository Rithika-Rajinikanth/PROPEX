...
use crate::{error::AppError, models::{GeoSearchQuery, RegionMetrics, SearchQuery, SearchResult}, AppState};
...
pub async fn semantic_search(State(state): State<Arc<AppState>>, Query(query): Query<std::collections::HashMap<String, String>>) -> Result<Json<Vec<RegionMetrics>>, AppError> {
    let search_query = query.get("q").ok_or(AppError::BadRequest("Missing 'q' parameter".into()))?;
    let embedding_service = EmbeddingService::new();
    let query_embedding = embedding_service.generate_embedding(search_query).await
    ...
    ).bind(embedding_str).fetch_all(state.db.pool()).await?;
    Ok(Json(results))
}

#[utoipa::path(get, path = "/api/v1/search", params(SearchQuery), responses((status = 200, description = "Search results", body = SearchResult)))]
pub async fn search_properties(State(state): State<Arc<AppState>>, Query(params): Query<SearchQuery>) -> Result<Json<SearchResult>, AppError> {
    params.validate()?;
    ...
    ).fetch_one(state.db.pool()).await.unwrap_or(false);
...
async fn search_with_mv(state: &Arc<AppState>, params: &SearchQuery, limit: i64, offset: i64) -> Result<Json<SearchResult>, AppError> {
    // Build base query
    let base_select = 
    ...
    let query_str = format!(
        ...
        base_select, where_clause, param_index, param_index + 1
    );
    ...
    tracing::info!(
        ...
        results.len(), params.state, params.region_type, params.heat_index_min, params.price_min, params.price_max
    );
...
async fn search_direct(state: &Arc<AppState>, params: &SearchQuery, limit: i64, offset: i64) -> Result<Json<SearchResult>, AppError> {
    let base_query = r#"
    ...
    if let Some(ref location) = params.location {
        conditions.push(format!("LOWER(r.region_name) LIKE LOWER(${})", bindings.len() + 1));
    ...
    let query_str = format!(
        ...
        base_query, where_clause, bindings.len() + 1, bindings.len() + 2
    );
    ...
    Ok(Json(SearchResult { total, page: offset / limit, page_size: limit, results }))
}
...
pub async fn advanced_search(State(state): State<Arc<AppState>>, Json(params): Json<SearchQuery>) -> Result<Json<SearchResult>, AppError> {
...
pub async fn geospatial_search(State(_state): State<Arc<AppState>>, Json(params): Json<GeoSearchQuery>) -> Result<Json<Vec<RegionMetrics>>, AppError> {
    params.validate()?;
    Err(AppError::BadRequest("Geospatial search requires PostGIS and geometry data.".to_string()))
}